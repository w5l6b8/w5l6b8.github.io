...
Ext.define('app.view.platform.design.filterScheme.FilterSpanForm', {
	...
	listeners : {
		show : function(w) {
			...
			if (!w.treeRecord.get('leaf'))
				if (!w.treeRecord.get('xtype'))
					form.findField('xtype').setValue('container');
		...
		this.items = [{
			...
			buttons : [{
				...
				handler : function(button) {
					...
					if (this.treeRecord.get('leaf') == false)
						this.treeRecord.set('text', this.treeRecord
										.get('title'));
					this.close();
				}
			}, {
							...
							items : [{
								...
								store : {
									data : UserFilterUtils.numberFieldOperator
											.concat(UserFilterUtils.stringFieldOperator)
								}
							}, {
								xtype : 'checkbox',
								margin : '0 0 0 10',
