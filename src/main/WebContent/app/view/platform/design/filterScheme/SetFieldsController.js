...
Ext.define('app.view.platform.design.filterScheme.SetFieldsController', {
			...
			saveToFilterScheme : function(objectid, schemeid, schemename,
					mydefault, shareowner, shareall, createorupdatewindow) {
				var me = this;
				var selectedTree = this
						.lookupReference('moduleselectedfieldstree');
				...
				Ext.Ajax.request({
							...
							params : {
								...
								shareall : shareall,
								schemeDefine : Ext.encode(this
										.getChildNodesArray(rootNode))
							},
							success : function(response) {
								...
								if (result.success) {
									EU
											.toastInfo('筛选方案『' + schemename
													+ '』已保存。');
									createorupdatewindow
											.afterSaveScheme(result.tag);
			...
			getChildNodesArray : function(pnode) {
				...
				Ext.each(pnode.childNodes, function(node) {
							...
							if (node.get('title'))
								nodedata.title = node.get('title');
							if (node.isLeaf() && node.get('itemId'))
								nodedata.itemId = node.get('itemId');
							var farray = ['xtype', 'rows', 'cols', 'widths',
									'rowspan', 'colspan', 'filtertype',
									'operator','hiddenoperator', 'othersetting', 'remark'];
							Ext.each(farray, function(f) {
										if (node.get(f))
											nodedata[f] = node.get(f);
									})
							if (!node.isLeaf())
								nodedata.children = me.getChildNodesArray(node);
				...
				Ext.Ajax.request({
							...
							success : function(response) {
								...
								selectedTree.getRootNode()
										.appendChild(resp.children);
							}
						})
			},

			columnEditAction : function(grid, rowIndex, colIndex, item, e,
					record) {
				var rec = grid.getStore().getAt(rowIndex);
				grid.getSelectionModel().select(rec);
				Ext.create(
						'app.view.platform.design.filterScheme.FilterSpanForm',
						{
							treeRecord : record
						}).show();
