...
Ext.define('app.view.platform.module.userFilter.TextFilter', {
			...
			initComponent : function() {
				...
				if (!defaultoperator)
					defaultoperator = UserFilterUtils.stringFieldOperator[0];
				...
				me.items = [{
							...
							listeners : {
								change : function(combo, value) {
									combo.nextNode().setEmptyText(combo
											.getRawValue());
								}
							},
							store : {
							...
							triggers : {
								clear : {
									...
									handler : function(field) {
										field.setValue(null);
										field.up('moduleuserfilter')
												.executeFilterForChange();
									}
								}
							},
							listeners : {
							...
							store : {
								fields : ['text'],
								data : UserFilterUtils
										.getHistory(me.userfilter.fieldid)
							}
						}]
			...
			getFilter : function() {
				...
				if (!value)
					return null;
				valueCombo.getStore().setData(UserFilterUtils.addItemToHistory(
						me.userfilter.fieldid, value));
