...
Ext.define('app.view.platform.module.attachment.ModuleController', {
	  ...
	  onViewSelectionChange : function(view, selected){
		  ...
		  var url = "platform/attachment/preview.do?attachmentid=" + model.get('attachmentid');
		  if (model.get('originalpreviewmode') == 'image') {
			  previewPanel.setActiveItem(1);
			  var imagepanel = me.lookupReference('imagepreviewpanel');
			  imagepanel.setImage(url, model.get('pwidth'), model.get('pheight'));
			  me.lasturl = url;
			  me.lastmodel = model;
			  me.lasttype = 'image';
		  } else if (model.get('originalpreviewmode') == 'direct' || model.get('haspdfpreviewviewdata')) {
	  ...
	  downloadCurrent : function(){
		  ...
		  if (model.get('filename')) {
			  window.location.href = 'platform/attachment/download.do?attachmentid=' + model.get('attachmentid');
		  } else EU.toastWarn("当前选中的附件尚未上传附件文件！");
	  },

	  downloadAll : function(){
		  var pf = this.getView().parentFilter;
		  if (pf.fieldvalue) window.location.href =
		      'platform/attachment/downloadall.do?moduleName=' + pf.moduleName + '&idkey=' + pf.fieldvalue;
		  else EU.toastWarn('当前记录尚未保存，请先保存后再执行附件操作!');
	  },
		  ...
		  if (me.lasturl) {
			  ...
			  var htmlMarkup =
			      [
			          ...
			          me.lasttype == 'image' ? '<img src="' + basePath + me.lasturl + '"/>' : '<iframe src="' + basePath
			              + me.lasturl + '" ></iframe>',
