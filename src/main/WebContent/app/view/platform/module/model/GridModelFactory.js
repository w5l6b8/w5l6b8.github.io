...
Ext.define('app.view.platform.module.model.GridModelFactory', {
	  ...
	  requires : ['app.view.platform.module.model.CommonFunction', 'Ext.data.identifier.Sequential'],
	  ...
	  statics : {
		  ...
		  getModelByModule : function(module){
			  var model = Ext.define('app.model.' + module.fDataobject.objectname, {
				    ...
				    mixins : {
					    commonFunction : 'app.view.platform.module.model.CommonFunction'
				    },
					    ...
					    switch (field.fieldDefine.fieldtype.toLowerCase()) {
						    case 'date' :
							    ...
							    field.convert = function(v){
								    ...
								    if (dateFormat) { return Ext.Date.parse(v, dateFormat); }
								    parsed = Date.parse(v);
						    ...
						    case 'datetime' :
						    case 'timestamp' :
							    field.dateWriteFormat = 'Y-m-d H:i:s';
		  ...
		  getField : function(fd){
			  ...
			  } else if (fd.isAdditionField) {
				  var field = {
					  fieldDefine : fd,
					  orginalField : fd.orginalField,
					  name : fd.fieldname,
					  title : fd.fieldtitle,
					  type : this.getTypeByStr(fd.orginalField.fieldtype),
					  persist : false
				  }

				  if (fd.tf_aggregate === 'normal') {
					  // 父模块的附加字段
					  if (!this.hasContainerField(field, fields)) fields.push(field);
					  // 附加的父模块的主键字段
					  var additionIdField = {
						  fieldDefine : null,
						  orginalField : null,
						  name : fd.parentKeyField,
						  type : 'string',
						  persist : false
					  }
					  if (!this.hasContainerField(additionIdField, fields)) fields.push(additionIdField);
				  } else {
					  // 所有子模块的附加字段
					  fields.push(field);
				  }
			  } else if (fd.isAdditionField || fd.isOneToMany) {
				  var field = {
					  fieldDefine : fd,
					  type : 'int',
					  persist : false
