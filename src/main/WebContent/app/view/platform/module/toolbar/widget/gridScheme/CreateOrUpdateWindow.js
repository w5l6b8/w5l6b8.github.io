...
Ext.define('app.view.platform.module.toolbar.widget.gridScheme.CreateOrUpdateWindow', {
	extend : 'Ext.window.Window',
	alias : 'widget.gridschemecreateupdate',
	requires : [ 'Ext.layout.container.Card', 'app.view.platform.design.columnScheme.SetFields' ],
	layout : 'card',
	width : '90%',
	height : '90%',
	modal : true,
	...
	initComponent : function() {
		...
		me.titleAhead = me.actionTitle + '列表方案', me.iconCls = 'x-fa fa-' + (me.isCreate ? 'plus' : 'edit');
					...
					items : [
							{
								xtype : 'container',
								height : 130,
								html : '<h2>你正在对『' + me.moduleInfo.modulename + '』' + me.actionTitle + '一个列表方案</h2><p>请先录入以下信息</p>'
										+ '<p>然后再按‘下一步’继续......'
	...
	showNext : function() {
		...
		if (me.down('form').isValid()) {
			...
			if (me.moduleInfo.checkGridSchemeNameValidate(this.gridSchemeId, name))
	...
	saveScheme : function() {
		...
		this.down('setgridfields').getController().saveToGridScheme(this.moduleInfo.fDataobject.objectid,
				this.gridSchemeId, this.down('textfield#schemename').getValue(), this.down('field#mydefault').getValue(),
				this.down('field#shareowner').getValue(), this.down('field#shareall').getValue(), this)
	},
	...
	doCardNavigation : function(incr) {
		...
		this.setTitle(this.titleAhead + (next == 0 ? ' 设置列表方案名称' : ' 选择列表方案字段'));
