...
Ext.define('app.view.platform.module.toolbar.widget.gridScheme.MenuButtonController', {
	...
	requires : [ 'app.view.platform.module.toolbar.widget.gridScheme.CreateOrUpdateWindow',
			'app.view.platform.module.toolbar.widget.gridScheme.SaveAsWindow' ],
	...
	onMenuShow : function() {
		...
		view.down('#currentname').setText(
				view.down('#currentname').aheadtext + '<span style="color:blue;">' + view.grid.currentGridScheme.schemename)
				+ "</span>";
	...
	setCurrentToDefault : function() {
		...
		Ext.Ajax.request({
			url : 'userfavourite/setdefaultgridscheme.do',
			params : {
				schemeid : id
			},
	...
	editScheme : function() {
		var view = this.getView(), grid = view.up('tablepanel');
		Ext.widget('gridschemecreateupdate', {
			isCreate : false,
		...
		Ext.Ajax.request({
			...
			success : function(response) {
				...
				if (isCreate) {
					...
					grid.fireEvent('gridSchemeModified', result);
			}
		})
	},
	...
	afterDeleteScheme : function(scheme) {
		...
		if (items.items.length == 0)
			this.getView().getMenu().remove(this.getView().down('menuitem#ownergroup'));
	...
	deleteScheme : function() {
		...
		if (grid.moduleInfo.getGridSchemeCount() == 1) {
			EU.toastWarn('只剩最后一个方案了，再删就连毛都没有了！！！');
			return;
		}
