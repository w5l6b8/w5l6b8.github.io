...
Ext.define('app.view.platform.module.grid.column.ManyToMany', {
	  ...
	  initComponent : function(){
		  ...
		  var joinTable = this.fieldDefine.jointable;
		  // 取得joinTable的模块定义
		  var joinModule = modules.getModuleInfo(joinTable);

		  // manyToMany 另一端的模块名称，模块的字段名为Set<modulename>,或
		  // List<module>,利用正则表达式，取得<>之间的内容。
		  this.manyToManyModuleName = /\w+/.exec(/<\w+>/.exec(this.fieldDefine.fieldtype)[0])[0];
		  this.manyToManyModuleTitle = modules.getModuleInfo(this.manyToManyModuleName).fDataobject.title;
		  ...
		  if (val) {
			  ...
			  for (var i in records) {
				  if (records[i]) {
					  ...
					  result +=
					      '<span class="manyToManyContext" _id="' + fields[0] + '" _joinid="' + fields[2] + '">' + fields[1]
	  ...
	  processEvent : function(type, view, cell, recordIndex, cellIndex, e, record, row){
		  ...
		  if (type === 'click') {
			  ...
			  if (e.getTarget().className === 'manyToManyContext') {
				  ...
				  var text =
				      module.fDataobject.title + ' ' + record.getTitleTpl() + ' 的 ' + this.manyToManyModuleTitle + '『' + title
				          + '』';
