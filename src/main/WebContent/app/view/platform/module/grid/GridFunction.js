...
Ext.define('app.view.platform.module.grid.GridFunction', {
	extend : 'Ext.Mixin',
	...
	refreshAll : function(){
		var me = this;
		me.store.reload();
		me.modulePanel.getModuleNavigate().refreshNavigateTree();
	...
	setParentFilter : function(fp){
		var me = this;
		me.parentFilter = fp;
		this.getStore().parentFilter = me.parentFilter;
	...
	gridSchemeChange : function(scheme){
		...
		me.columns = app.view.platform.module.grid.ColumnsFactory.getColumns(me.moduleInfo, scheme);
	...
	updateTitle : function(){
		...
		if (me.parentFilter) {
			title =
			    title + ' 『' + me.parentFilter.fieldtitle + ':'
		...
		if (me.view) {
			...
			if (selected) if (!!selected.getTitleTpl()) title =
			    title + '　『<em>' + selected.getTitleTpl() + '</em>'
			        + (selecteds.length > 1 ? ' 等' + selecteds.length + '条' : '') + '』';
		}

		if (me.inWindow) me.up('window').setTitle(title);
	...
	getExportGridColumns : function(){
		if (this.lockedGrid) {
			return this._getExportGridColumns(this.lockedGrid.headerCt.items.items).concat(this._getExportGridColumns(this.normalGrid.headerCt.items.items));
		} else {
	...
	selectNextRecord : function(){
		...
		if (sm.getCount() == 0) {
			...
			var index = store.indexOf(sm.getSelection()[0]);
			if (index + ((store.currentPage - 1) * store.pageSize) == store.getTotalCount() - 1) EU.toastWarn('已经是当前列表的最后一条记录!');
			else {
				if (store.buffered) {
					sm.select(store.getAt(index + 1));
				} else {
