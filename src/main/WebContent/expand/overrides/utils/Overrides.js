...
Ext.apply(Ext.util.Format, {
	addDataToolTip : function(val, metaData, model, row, col, store, gridview){
		if (gridview) {
			...
			try {
				if (column && column.tooltipXTemplate) metaData.tdAttr = 'data-qtip="' + column.tooltipXTemplate.apply(model.data) + '"';
			} catch (e) {
				console.log(e);
			}
		}
	...
	monetaryRenderer : function(val, metaData, model, row, col, store, gridview){
		if (val) {
			if (val) {
				if (Ext.monetaryUnit && Ext.monetaryUnit != 1) val = val / Ext.monetaryUnit;
	...
	intRenderer : function(val, metaData, model, row, col, store, gridview){
		...
		if (gridview) var column = gridview.ownerCt.columnManager.getHeaderAtIndex(col);
	...
	percentRenderer : function(v, metaData, model, row, col, store, gridview){
		...
		Ext.util.Format.addDataToolTip(v, metaData, model, row, col, store, gridview);
		var colors = [
		    '#DDE7EC',
		    '#CADFE9',
		    '#B7D6E5',
		    ...
		    '#46A2D0',
		    '#3399CC'];
		return Ext.String.format('<div>' + '<div style="float:left;border:1px solid #C0C0C0;height:18px;width:100%;">' + '<div style="float:left;text-align:center;width:100%;color:blue;">{0}%</div>' + '<div style="background: {2};width:{1}%;height:16px;"></div>' + '</div></div>', v, v1, '#B7D6E5'
	...
	wavgRenderer : function(v, metaData, model, row, col, store, gridview){
		if (!Ext.isNumeric(v)) {
			...
			return Ext.String.format('<div>' + '<div style="float:left;border:1px solid #C0C0C0;height:18px;width:100%;">' + '<div style="float:left;text-align:center;width:100%;color:blue;"></div>' + '<div style="background: {2};width:{1}%;height:16px;"></div>' + '</div></div>', 0, 0, '#B7D6E5');
		}
		var column = gridview.ownerCt.columnManager.getHeaderAtIndex(col);
		fz = model.get(column.dataIndex + '1'), fm = model.get(column.dataIndex + '2');
		v = Math.round(v * 10000) / 100;
		var v1 = v > 100 ? 100 : v;
		v1 = v1 < 0 ? 0 : v1;
		metaData.tdAttr = 'data-qtip="&nbsp;&nbsp;&nbsp;' + Ext.util.Format.number(fz, '0,000.00') + '<br/>' + "<hr color=#fff size=1>" + '&nbsp;&nbsp;&nbsp;' + Ext.util.Format.number(fm, '0,000.00') + '&nbsp;&nbsp;&nbsp;"';
		return Ext.String.format('<div>' + '<div style="float:left;border:1px solid #C0C0C0;height:18px;width:100%;">' + '<div style="float:left;text-align:center;width:100%;color:blue;">{0}%</div>' + '<div style="background: {2};width:{1}%;height:16px;"></div>' + '</div></div>', v, v1, '#B7D6E5');
	},

	// 数据挖掘机中显示sum的值中，如果有父节点的值，那就显示关于父节点的百分比
	aggregateSumRenderer : function(val, metaData, model, row, col, store, gridview){
		...
		if (pnode.get(column.dataIndex)) {
			var bl = Math.round((!!val ? val : 0) / pnode.get(column.dataIndex) * 10000) / 100;
			var bl1 = bl > 100 ? 100 : bl;
			bl1 = bl1 < 0 ? 0 : bl1;
			metaData.tdAttr = 'data-qtip="占 ' + (pnode.get('text_') || pnode.get('text')) + ' 的 ' + bl + '%"';
			return Ext.String.format('<div>' + '<div style="float:left;height:18px;width:100%;">' + '<div style="float:left;text-align:right;width:100%;color:blue;">{0}</div>' + '<div style="background: {2};width:{1}%;height:2px;"></div>' + '</div>', Ext.util.Format.monetaryRenderer(val, metaData, model, row, col, store, gridview), bl1, '#DDE7EC');
		} else {
	...
	aggregateSumFloatRenderer : function(val, metaData, model, row, col, store, gridview){
		...
		if (pnode.get(column.dataIndex)) {
			...
			bl1 = bl1 < 0 ? 0 : bl1;
			metaData.tdAttr = 'data-qtip="占 ' + (pnode.get('text_') || pnode.get('text')) + ' 的 ' + bl + '%"';
			return Ext.String.format('<div>' + '<div style="float:left;height:18px;width:100%;">' + '<div style="float:left;text-align:right;width:100%;color:blue;">{0}</div>' + '<div style="background: {2};width:{1}%;height:2px;"></div>' + '</div>', Ext.util.Format.floatRenderer(val, metaData, model, row, col, store, gridview), bl1, '#DDE7EC');
		} else {
			return Ext.util.Format.floatRenderer(val, metaData, model, row, col, store, gridview)
		}
	},

	// 数据挖掘机中显示count的值中，如果有父节点的值，那就显示关于父节点的百分比
	aggregateCountRenderer : function(val, metaData, model, row, col, store, gridview){
		...
		if (pnode.get(column.dataIndex)) {
			...
			bl1 = bl1 < 0 ? 0 : bl1;
			metaData.tdAttr = 'data-qtip="占 ' + (pnode.get('text_') || pnode.get('text')) + ' 的 ' + bl + '%"';
			return Ext.String.format('<div>' + '<div style="float:left;height:18px;width:100%;">' + '<div style="float:left;text-align:right;width:100%;color:blue;">{0}</div>' + '<div style="background: {2};width:{1}%;height:2px;"></div>' + '</div>', Ext.util.Format.intRenderer(val, metaData, model, row, col, store, gridview), bl1, '#DDE7EC');
		} else {
