			...
			if (mystore && Ext.isEmpty(mystore.data)) {
				...
				if (!Ext.isEmpty(mystore.proxy)) {
					mystore.proxy.reader.rootProperty = function(data){
						if (!Ext.isEmpty(data) && me.paging) return data.data;
						return data;
					};
				}
				if (me.paging) {
					mystore.pageSize = mystore.pageSize || 20;
					mystore.remoteSort = Ext.isEmpty(mystore.remoteSort) ? true : mystore.remoteSort, mystore.proxy.extraParams = Ext.apply(mystore.proxy.extraParams, {
						paging : true
					});
