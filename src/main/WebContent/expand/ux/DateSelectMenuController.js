		...
		this.control({
			...
			'form#yearsection numberfield[name=firstyear]' : {
				change : function(field, value) {
					var ly = field.up('form')
							.down('numberfield[name=lastyear]');
					if (ly.getValue() < value)
						ly.setValue(value);
				}
			},
			'form#yearsection numberfield[name=lastyear]' : {
				change : function(field, value) {
					var fy = field.up('form')
							.down('numberfield[name=firstyear]');
					if (fy.getValue() > value)
						fy.setValue(value);
				}
			},
			...
			'form button' : {
				click : function(button) {
					...
					switch (form.itemId) {
						...
						case 'yearsection' :
							if (firstyearfield.disabled
									|| lastyearfield.disabled) {
						...
						case 'monthsection' :
							firstmonth = form.getForm().findField('firstmonth')
									.getValue();
							lastmonth = form.getForm().findField('lastmonth')
									.getValue();
							...
							if (firstyearfield.disabled
								...
								if (!firstyearfield.disabled) { // lastyear 不可用
									result.value = firstyear + '-' + firstmonth
											+ '--';
									text = '从' + firstyear + '年' + firstmonth
											+ '月起';
								} else {
									result.value = '--' + lastyear + '-'
											+ lastmonth;
									text = '至' + lastyear + '年' + lastmonth
											+ '月止';
								}
							} else {
								if (firstyear == lastyear) {
									if (firstmonth == lastmonth) {
										text = firstyear + '年' + firstmonth
												+ '月';
										result.value = firstyear + '-'
												+ firstmonth;
							...
							break;
						case 'quartersection' :
							firstquarter = form.getForm()
									.findField('firstquarter').getValue();
							lastquarter = form.getForm()
									.findField('lastquarter').getValue();
							if (firstyearfield.disabled
									|| lastyearfield.disabled) {
								if (!firstyearfield.disabled) { // lastyear 不可用
									result.value = firstyear + '-'
											+ firstquarter + '--';
									text = '从' + firstyear + '年' + firstquarter
											+ '季起';
								} else {
									result.value = '--' + lastyear + '-'
											+ lastquarter;
									text = '至' + lastyear + '年' + lastquarter
											+ '季止';
								}
							} else {
								if (firstyear == lastyear) {
									if (firstquarter == lastquarter) {
										text = firstyear + '年' + firstquarter
												+ '季';
										result.value = firstyear + '-'
												+ firstquarter;
			...
			'form#monthsection numberfield[name=firstyear]' : {
				change : function(field, value) {
					...
					if (ly.getValue() < value)
						...
						var fm = field.up('form')
								.down('numberfield[name=firstmonth]')
								.getValue();
						var lmf = field.up('form')
								.down('numberfield[name=lastmonth]');
						if (lmf.getValue() < fm)
							lmf.setValue(fm);
					}
				}
			...
			'form#monthsection numberfield[name=lastyear]' : {
				change : function(field, value) {
					...
					if (fy.getValue() > value)
						...
						var lm = field.up('form').down('[name=lastmonth]')
								.getValue();
			...
			'form#monthsection numberfield[name=firstmonth]' : {
				change : function(field, value) {
					var ly = field.up('form').down('[name=lastyear]')
							.getValue();
					var fy = field.up('form').down('[name=firstyear]')
							.getValue();
					if (ly == fy) {
						var lmf = field.up('form').down('[name=lastmonth]');
						if (lmf.getValue() < value)
							lmf.setValue(value);
					}
				}
			},
			...
			'form#quartersection numberfield[name=firstyear]' : {
				change : function(field, value) {
					...
					if (ly.getValue() < value)
						...
						var fm = field.up('form')
								.down('numberfield[name=firstquarter]')
								.getValue();
						var lmf = field.up('form')
								.down('numberfield[name=lastquarter]');
						if (lmf.getValue() < fm)
							lmf.setValue(fm);
					}
				}
			...
			'form#quartersection numberfield[name=lastyear]' : {
				change : function(field, value) {
					...
					if (fy.getValue() > value)
						...
						var lm = field.up('form').down('[name=lastquarter]')
								.getValue();
			...
			'form checkbox[name=_enablefirst]' : {
				change : function(field, value) {
					...
					if (field.nextSibling().nextSibling())
						field.nextSibling().nextSibling()[value
								? 'enable'
								: 'disable']();
					if (!value) {
						field.up('form').down('checkbox[name=_enablelast]')
								.setValue(true);
					}
				}
			},
				...
				change : function(field, value) {
					...
					if (field.nextSibling().nextSibling())
						field.nextSibling().nextSibling()[value
								...
								: 'disable']();
					if (!value) {
						field.up('form').down('checkbox[name=_enablefirst]')
								.setValue(true);
					}
				}
			}
